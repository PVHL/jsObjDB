...
ObjIndex.prototype.toJSON = function() {
    return JSON.stringify({ unique: this.unique, required: this.required, property: this.property });
}
 ...
 * @param {Boolean} [async]    Should delegates and callbacks be called asynchronously? Defaults to false.
 * 
 * @example
 * var db = jsObjDB();
...
var jsObjDB = function (prim_key, async) {
    ...
    this._count = 0;
    //  Delegates that get notified on events. They are called in the order added for each event.
    this.delegates = [];
...
jsObjDB.prototype.loadFromJSON = function(json) {
    ...
    _.each(data.data, function(item) { delete item._id; });
...
jsObjDB.prototype.toJSON = function() {
    var dbDesc = { async: this.async, prim_key: this.prim_key };
    dbDesc.indexes = [];
    
    dbDesc.indexes = _.map(this.indexes, function(index) { 
        return { unique: index.unique, required: index.required, property: index.property };
    });
...
jsObjDB.prototype.sizeof = function () {
    var size = 0;
    size += 2 * 1 /* booleans */ 
            + (this.prim_key? 2 * this.prim_key.length : 0) /* string prim key name */ 
            + 8 * 2 /* numbers (ref_id, count) */ ;
 ...
 * </ul>
 * @example
 * db.addDelegate("deleg1", function(event) {
 ...
 *     ...
 * });
 * @param {String|Number} reg_id   An ID of this delegate. Used for quick delete
 * @param {Function}      delegate The function to call on any change
 * @param {Object}        binding  The object to bind the function to. (optional - defaults to "this")
 */
jsObjDB.prototype.addDelegate = function (reg_id, delegate, binding) {
    if (binding) {
        this.delegates.push({
            id: reg_id,
            fn: _.bind(delegate, binding)
        });
    } else {
        this.delegates.push({
            id: reg_id,
 ...
 *
 * @param   {String|Number}    reg_id The reference for the delegate to be deleted.
 */
jsObjDB.prototype.removeDelegate = function (reg_id) {
    _.remove(this.delegates, function (item) {
        return item.id === reg_id;
    });
}

/**
 * Call all delegates, passing them information about the event. Depending on the 
 ...
 */
jsObjDB.prototype.callDelegates = function (event) {
    //  Let our delegates know that something has happened
